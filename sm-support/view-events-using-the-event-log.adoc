...
|===
. *Optional*: Um neue Ereignisse aus dem Speicher-Array abzurufen, klicken Sie auf **Aktualisieren**.
+
Es kann einige Minuten dauern, bis ein Event protokolliert wird und auf der Seite *Ereignisprotokoll* angezeigt wird.
...
. *Optional*: Zum Löschen von Ereignissen aus dem Ereignisprotokoll:
+
